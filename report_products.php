...
<body>
	...
	<?php $sql = 'select p.PID, u.unitTitle,p.PTitle, sum(case when h.ttype=1 or h.ttype=3 or h.ttype=5 then (quantity) else  (-1*quantity)  end ) Balance  from transactionsbody b join transactionsheader h on h.THID=b.THID join products p on p.PID=b.PID join units u on u.unitID=b.unitID 
    GROUP by b.PID,b.unitID  order by h.THID desc';
$Transactiondata = $pdo->query($sql)->fetchAll();

	echo '<script> var JsProductsData='.json_encode($Transactiondata).';  </script>'; ?>
						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item active" aria-current="page">تقرير الاصناف</li>
								</ol>
							</nav>
						</div>
        ...
        <div class="clearfix">
            <form id="fastSearchForm" >
				...
				<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
					...
					<div class="row">
						<table class="stripe hover multiple-select-row data-table-export nowrap" id="ProductsReport">
	...
	<script>
		$('document').ready(function(){
			var buildDataTable=function(PrdData)
			{
			$("#ProductsReport").DataTable({
				paging: false,
		...
		$('#fastSearchForm').on('submit',function(e){
			...
			console.log(dataS);
			$.post('serversideCalls/Ajax/Reports/ListProducstBalances/',$(this).serializeArray(),function(r){
				console.log(r);
				 $("#ProductsReport").DataTable().clear().destroy();
