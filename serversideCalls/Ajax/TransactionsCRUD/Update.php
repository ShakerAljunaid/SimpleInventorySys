...
if($param["type"]==2 || $param["type"]==4)
{
	 ...
	 if($valid)
	 {
		$fields=array(":vId",":vuserID",":vbID",":vwhID",":vvID_CID",":vtType",":vvoucherDate",":vcomment");
        $values=array( check_data($param["THID"]),check_data($_SESSION["userID"]),check_data($param["BID"]),check_data($param["WhID"]),check_data($param["VID_CID"]),check_data($param["type"]),check_data($param["VoucherDate"]),check_data($param["Comment"]));
         $result=bind_fields('UpdateTranscationHeader',$fields,$values);

            	 
		 for($i=0;$i<count($someArray);$i++)
            { 
		$rate=empty(check_data($someArray[$i]["rate"]))?1:check_data($someArray[$i]["rate"]);
		      if(checkRowExistence(check_data($someArray[$i]["rowId"]),check_data($param["THID"]))==0)
			   {
	 		 $fields=array(":THID",":rowId",":userID",":PID",":unitID",":quantity",":rate",":gross",":narration");	
              $values=array( check_data($param["THID"]),check_data($someArray[$i]["rowId"]),check_data($_SESSION["userID"]),check_data($someArray[$i]["PID"]),check_data($someArray[$i]["unitID"]),check_data($someArray[$i]["quantity"]),$rate,empty(check_data($someArray[$i]["gross"]))?($rate*check_data($someArray[$i]["quantity"])):check_data($someArray[$i]["gross"]),check_data($someArray[$i]["narration"]));
               echo bind_fields('AddTransactionBody',$fields,$values);
			   }
			   else 
			   {$fields=array(":bTHID",":bRowId",":BPID",":BunitID",":Quantity",":Rate",":Gross",":Narration");
		           $values=array( check_data($param["THID"]),check_data($someArray[$i]["rowId"]),check_data($someArray[$i]["PID"]),check_data($someArray[$i]["unitID"]),check_data($someArray[$i]["quantity"]),$rate,empty(check_data($someArray[$i]["gross"]))?($rate*check_data($someArray[$i]["quantity"])):check_data($someArray[$i]["gross"]),check_data($someArray[$i]["narration"]));
                   echo bind_fields('UpdateTransactionBody',$fields,$values);
			   }
...
function checkRowExistence($bRowId,$bTHID)
{
 $sql="select bodyID  from transactionsbody  where  rowId=".$bRowId." and THID=".$bTHID;
   $bodyID = current($GLOBALS['pdo']->query($sql)->fetchAll());
 
 return !empty($bodyID['bodyID'])?$bodyID['bodyID']>0?$bodyID['bodyID']:0:0;
